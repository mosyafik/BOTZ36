...
case 'daftar':
if (cekUser("id", sender) !== null) return reply("😂 Kamu sudah terdaftar sebelum nya, Gunakan command #menu/.menu untuk mengetahui apa saja fungsi bot ini")
daftar.push({id: sender, emote:"✅", ban:false})
fs.writeFileSync('./db/function/daftar.json', JSON.stringify(daftar))
reply(`[ *🤝SELAMAT ANDA TELAH TERDAFTAR🤝* ] 
...
nayla.sendMessage(from, {image:{url:x5}, caption:"Ditemukan✅", mentions:[sender]},{quoted:nay1})
});
break
case 'pin': case 'pinterest': 
if (cekUser("id", sender) == null) return reply("Kamu belum terdaftar di database bot, Silahkan daftar terlebih dahulu, gunakan command #daftar")
if (!q) return reply("Masukkan nama karakter anime")
reply("[❗] Ini Dia Fotonya")
...
nayla.sendMessage(from, {image:{url:x7}, caption:"Ditemukan✅", mentions:[sender]},{quoted:nay1})
});
break
case 'ssweb': case 'ss': 
if (cekUser("id", sender) == null) return reply("Kamu belum terdaftar di database bot, Silahkan daftar terlebih dahulu, gunakan command #daftar")
...
const x28 = ["Mungkin Iya", "Mungkin Tidak", "Bisa Jadi", "Pasti lah", "Jelasss", "Woiyalah","Gak Banget", "Nggak lah","Kurang Tawu","Kayaknya Begitu"][Math.floor(Math.random() * (["Mungkin Iya", "Mungkin Tidak", "Bisa Jadi", "Pasti lah", "Jelasss", "Woiyalah","Gak Banget", "Nggak lah","Kurang Tawu","Kayaknya Sih"].length))]
reply(`[ *KERANG-AJAIB* ]
• *Cmd* : ${command}
• *Pertanyaan* : ${q}
• *Jawaban* : ${x28}`)
...
if (cekUser("id", sender) == null) return reply("Kamu belum terdaftar di database bot, Silahkan daftar terlebih dahulu, gunakan command #daftar")
if (!args[0]) return reply("Masukkan Text")
const x29 = ["Gatau","Besok","Kemaren","Gabakalan Coy","Minggu depan","Bulan depan","Tahun depan","Lusa","Setelah Kiamat"][Math.floor(Math.random() * (["Gatau","Besok","Kemaren","Gabakal","Minggu depan","Bulan depan","Tahun depan","Lusa","Setelah Kiamat"].length))]
reply(`[ *KERANG-AJAIB* ]
• *Cmd* : ${command}
• *Pertanyaan* : ${q}
• *Jawaban* : ${x29}`)
...
if (cekUser("id", sender) == null) return reply("Kamu belum terdaftar di database bot, Silahkan daftar terlebih dahulu, gunakan command #daftar")
if (!args[0]) return reply("Masukkan Text")
const x30 = ["Bisa aja","Tidak","Mustahil","Mana Bisa Gitu"][Math.floor(Math.random() * (["Bisa","Tidak","Mustahil", "Mana Bisa Gitu"].length))]
reply(`[ *KERANG-AJAIB* ]
• *Cmd* : ${command}
• *Pertanyaan* : ${q}
• *Jawaban* : ${x30}`)
